...
internal fun Date.isTheSameDay(comparedDate: Date): Boolean {
    ...
    return calendar.get(Calendar.DAY_OF_YEAR) == comparedCalendarDate.get(
        Calendar.DAY_OF_YEAR) && calendar.get(
        Calendar.YEAR
...
internal fun Date.compareDate(comparedDate: Date): Int {
    val calendar = Calendar.getInstance()
    calendar.withTime(this)
    val comparedCalendarDate = Calendar.getInstance()
    comparedCalendarDate.withTime(comparedDate)
    return calendar.compareTo(comparedCalendarDate)
}